...
deseq_data <- function(raw_data) {

  transformed_data <- raw_data

  # this was the default value in the deseq_varstab function and it is not altered
  # elsewhere in the code
  sample_conditions <- rep("A", nsamples(raw_data))

  # Enforce orientation.
  if (!phyloseq::taxa_are_rows(raw_data)) {
    raw_data <- t(raw_data)
  }

  x <- as(phyloseq::otu_table(raw_data), "matrix")
  ...
  # later
  tax_adf <- as(data.frame(as(phyloseq::tax_table(raw_data), "matrix"),
  ...
  otu_table(transformed_data) <- otu_table(vsmat, taxa_are_rows = TRUE)

  return(transformed_data)
}

deseq_otu_data <- lapply(raw_otu_data, deseq_data)
