...
read_tsv("data/simulation_clusters.tsv.gz") %>%
  ...
  filter((distance == "bray" &
            transform %in% c("proportion", "subsample15", "none", "deseq")) |
        (distance == "euclidean" &
            transform %in% c("none", "deseq", "subsample15")) |
        (distance == "poisson" &
            transform %in% c("subsample15", "none")) |
        (distance == "logFC" &
            transform %in% c("upperquartile", "subsample15")) |
        (distance == "uunifrac" &
            transform %in% c("subsample15", "proportion")) |
        (distance == "wunifrac" &
            transform %in% c("subsample15", "none", "proportion", "deseq"))
        ) %>%
...
ggsave(paste0("results/figures/my_figure_4.pdf"),
        width = 11, height = 10)
